...
#		@venues = Venue.select("DISTINCT(venues.name), venues.id, venues.name, venues.phone, venues.neighborhood_id, venues.file_name").joins(:neighborhood) # Having problems with using DISTINCT in postgreSQL db so using GROUP BY instead
...
#		@venues = @venues.select('venues.id, venues.name, venues.phone, venues.neighborhood_id, venues.file_name')
