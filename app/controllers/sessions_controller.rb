...
#		@venues = Venue.select("venues.id, venues.name, venues.phone, venues.neighborhood_id, venues.file_name").joins(:neighborhood)
		@venues = Venue.select("DISTINCT(venues.name) as venue_name, venues.name, venues.id, venues.phone, venues.neighborhood_id, venues.file_name").joins(:neighborhood) # Having problems with using DISTINCT in postgreSQL db so using GROUP BY instead		
...
#		@venues = @venues.group('venues.id, venues.name, venues.phone, venues.neighborhood_id, venues.file_name') # Using group method instead of select DISTINCT bc of problems in postgreSQL db
		@venues = @venues.order('venue_name').order(sort_order).limit(1) # Ensures only the most recent upcoming event is used for sorting (if user sorts by event start time).  Since this is only an activerecord relation, the query is not executed.
